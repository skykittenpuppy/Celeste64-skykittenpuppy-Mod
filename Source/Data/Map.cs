...
public class Map
{
	...
	public static readonly Dictionary<string, ActorFactory> ActorFactories =  new()
	{
		...
		["Badeline"] = new((map, entity) => new Badeline()),
		["Theo"] = new((map, entity) => new Theo()),
		["Maddy"] = new((map, entity) => new Maddy()),
		["Kit"] = new((map, entity) => new Kit()),
		["SignPost"] = new((map, entity) => new Signpost(entity.GetStringProperty("dialog", string.Empty))),
		["StaticProp"] = new((map, entity) =>
		{
			var prop = Path.GetFileNameWithoutExtension(entity.GetStringProperty("model", string.Empty));
	...
	private void LoadActor(World world, SledgeEntity entity)
	{
		...
		if (entity.ClassName == "PlayerSpawn")
		{
			...
			if (spawnsPlayer)
				HandleActorCreation(world, entity, new Player(Data.Worldspawn.GetStringProperty("playerModel", "maddy"),Data.Worldspawn.GetIntProperty("hasHair", 1) == 1), null);
...
>>>>>>> a95e581 (First commit):Source/Map.cs
=======
﻿using System.Runtime.InteropServices;
using Sledge.Formats.Map.Formats;
using SledgeMapObject = Sledge.Formats.Map.Objects.MapObject;
...
public class Map
{
	...
	private void LoadActor(World world, SledgeEntity entity)
	{
		...
		if (entity.ClassName == "PlayerSpawn")
		{
			...
			if (spawnsPlayer)
				HandleActorCreation(world, entity, 
				new Player(Data.Worldspawn.GetStringProperty("playerModel", "maddy"),
						Data.Worldspawn.GetIntProperty("hasHair", 1) == 1,
						Data.Worldspawn.GetIntProperty("hairCol0", 0x6ec0ff),
						Data.Worldspawn.GetIntProperty("hairCol1", 0xdb2c00),
						Data.Worldspawn.GetIntProperty("hairCol2", 0xfa91ff),
						Data.Worldspawn.GetIntProperty("hairColF", 0xf2d450)), null);
...
>>>>>>> 36c917d (Hair is now optional- properly):Source/Map.cs
